...
int main(int argc , char* argv[]){
    ...
    double gamma = atof(argv[4]); // decay rate (energy scale)
    ...
    bool compute_autocorrelation = false;
    ...
    string file_root_states = tinyformat::format("%sTN_purification_Ising_N%d_Jxx%.3f_hx%.3f_gamma%.3f_dt%.4f_D%d_cutoff%.2e_normalize%d",states_dir,N,Jxx,hx,gamma,dt,maxDim,cut_off,normalized);
    string file_root_data   = tinyformat::format("%sTN_purification_Ising_N%d_Jxx%.3f_hx%.3f_gamma%.3f_dt%.4f_D%d_cutoff%.2e",data_dir,N,Jxx,hx,gamma,dt,maxDim,cut_off);
    ...
    {
        string file_root = tinyformat::format("%sTN_purification_Ising_N%d_Jxx%.3f_hx%.3f_gamma%.3f_dt%.4f_D%d_cutoff%.2e_Tness%.1f",states_dir,N,Jxx,hx,gamma,dt,maxDim,cut_off,Tness);
        file_obs    = tinyformat::format("%s_Tness%.1f_Z1t_Z0.txt",file_root_data,Tness);
